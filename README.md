...
* 改规则有两个落笔的地方：正在运行的那份配置，改了马上有用，但订阅一更新它就被重新生成，你加的规则就跟着丢了；挂在订阅上的「编辑规则」文件（在 Verge 里右键订阅卡片能看到），订阅更新也冲不掉它，但只改这一处、不重新加载，当下不生效。
* 正在运行的那份是个几十万字符的 YAML 大文件，格式要求很严格，手动改错一个缩进整个文件就废了。
* 就算都改对了，浏览器和网站之间的旧连接还挂着，不重连看不到效果。

这个 skill 让 AI 替你把这些坑都绕开。你只管说人话，使用现场大概长这样：
...
* **两层都写、写在安全的位置**，你的规则存进订阅配套的「编辑规则」文件里，机场（卖代理服务的商家）更新订阅也冲不掉它
* **动手前先自动备份**，改坏了拿备份一还原就回来
* **改完立刻生效**，让正在运行的 Clash 马上重新加载，并掐断这个网站的旧连接，刷新页面就能看到效果（Clash 没开着也不丢：规则已写进文件，下次启动自动生效）
...
* **第一次用就报「当前订阅没有『编辑规则』链文件」？** 去 Clash Verge 右键你的订阅卡片，点「编辑规则」，什么都不用改、直接保存一次，再重跑命令。一次就好。
* **proxy 命令报「分组不存在」？** 默认往「🚀 节点选择」这个组里送，中文订阅大多叫这个名。分组就是 Clash Verge「代理」页里那些可以点的组名，你的订阅里叫什么去那一页看；名字不一样时，先执行 `export CLASH_RULE_PROXY_GROUP="你的分组名"`。
* **加了规则网站还是不对劲？** 先跑 `status`。它专查几种疑难杂症：比如 Clash 的内核（Verge 界面背后真正干活的部分）被切到了全局模式，所有分流规则整体失效，全部流量走一个节点；比如订阅的「全球直连」分组被指到了某个节点，所有本该直连的流量都被引去代理。每查出一个问题，它都会附上对应的修复命令。
